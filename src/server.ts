...
function kbSubgroups(catKey: string) {
  ...
  kb.text("⬅ Назад до груп", "menu:technique").row().text("🏠 На головну", "menu:root");
  return kb;
}
...
function kbExercisesInSub(catKey: string, subKey: string) {
  ...
  kb.text("⬅ Назад до підгруп", `cat:${catKey}`).row().text("🏠 На головну", "menu:root");
  return kb;
}
...
// назад на головну меню (тут краще також через editOrReply, бо виклик йде з медіа)
