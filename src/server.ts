...
bot.command("start", async (ctx) => {
  const photoId = "AgACAgIAAxkBAAE7GPRoxsMmhCGCvJRpJNXBs7XcyPjbGAACBfsxG8OBOEp8zGPvGXLsiwEAAwIAA3kAAzYE";
  const caption = "Привіт, тебе вітає: Дмитро — твій персональний тренер, чим можу допомогти?";
...
bot.callbackQuery("menu:services", async (ctx) => {
  await ctx.editMessageText("Оберіть послугу:", { reply_markup: kbServices() });
...
bot.callbackQuery("menu:root", async (ctx) => {
  await ctx.editMessageText("Оберіть розділ 👇", { reply_markup: kbMain() });
  await ctx.answerCallbackQuery();
});
